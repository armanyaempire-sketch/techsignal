...
export default async function ArticlePage({params}:{params:Promise<{slug:string}>}){
 ...
 {related.length>0&&<section className="related-section"><div className="section-header"><div><span className="eyebrow">Keep reading</span><h2>Related guides</h2></div></div><div className="related-grid">{related.map(x=><Link className="related-card" key={x.slug} href={"/articles/"+x.slug+"/"} prefetch={false}><span className="meta">{x.categoryName} · {x.stage}</span><h3>{x.title}</h3><p>{x.description}</p><span className="related-link">Read guide →</span></Link>)}</div></section>}
 <Smartlink articleSlug={a.slug}/><div className="article-end-ads"><NativeBanner slot={a.slug+"-native-bottom"} variant="horizontal"/><AdsterraBanner size="468x60" slot={a.slug+"-468-bottom"} visibility="desktop"/></div><AdSlot slot="bottom" scriptUrl={process.env.NEXT_PUBLIC_AD_BOTTOM_SCRIPT_URL} zone={process.env.NEXT_PUBLIC_AD_BOTTOM_ZONE}/></article>
