...
export default async function ArticlePage({params}:{params:Promise<{slug:string}>}){
 ...
 <AdStack position="article"/>{a.offerKey&&<AffiliateCTA articleSlug={a.slug} position="article-top" offerKey={a.offerKey}/>}<div className="article-prose" dangerouslySetInnerHTML={{__html:markdownToHtml(a.body)}}/>{a.offerKey&&<AffiliateCTA articleSlug={a.slug} position="article-bottom" offerKey={a.offerKey}/>}
 ...
 <AdSlot slot="bottom" scriptUrl={process.env.NEXT_PUBLIC_AD_BOTTOM_SCRIPT_URL} zone={process.env.NEXT_PUBLIC_AD_BOTTOM_ZONE}/></article>
 <aside className="sidebar"><div className="info-card"><span className="eyebrow">About this guide</span><h3>Search intent: {a.intent}</h3><p className="muted">This page is part of the {a.stage} editorial funnel.</p><Link className="text-link" href="/editorial-policy/">Read our editorial policy →</Link></div><AdSlot slot="sidebar" scriptUrl={process.env.NEXT_PUBLIC_AD_SIDEBAR_SCRIPT_URL} zone={process.env.NEXT_PUBLIC_AD_SIDEBAR_ZONE}/><div className="info-card"><span className="eyebrow">Editorial standard</span><p className="muted">No guaranteed results, fake testimonials or unsupported claims. Health articles are general information, not medical advice.</p></div></aside></div></>;
}
