...
export default function Home(){
 ...
 </div><div className="hero-card"><AdsterraBanner size="300x250" slot="home-hero-square"/></div></div></section>
 <div className="container"><div className="home-ad-stack"><NativeBanner slot="home-top-native-horizontal" variant="horizontal"/></div></div>
 <section className="section"><div className="container"><div className="section-header"><div><span className="eyebrow">Explore</span><h2>Three content desks</h2><p className="section-intro">Start with the subject that matches what you are trying to solve.</p></div></div><div className="grid-3">{categories.map(x=><Link className="card category-card" key={x.slug} href={"/category/"+x.slug+"/"} prefetch={false}><span className="eyebrow">{x.name}</span><h3>{x.name}</h3><p>{x.short}</p><strong>Explore desk <span aria-hidden="true">→</span></strong></Link>)}</div><div className="home-category-monetization"><AdsterraBanner size="300x250" slot="home-300-category-transition"/></div></div></section>
 <section className="section section-soft" id="latest"><div className="container"><div className="section-header"><div><span className="eyebrow">Latest</span><h2>New guides</h2></div></div><div className="home-latest-layout"><main className="home-latest-main"><div className="grid-3">{articles.map(x=><article className="card" key={x.slug}><div className="meta">{x.categoryName} · {x.stage}</div><h3><Link prefetch={false} href={"/articles/"+x.slug+"/"}>{x.title}</Link></h3><p>{x.description}</p></article>)}</div><div className="home-end-ad"><AdsterraBanner size="300x250" slot="home-300-end"/></div></main><aside className="home-latest-rail"><AdRail prefix="home-rail"/></aside></div><Smartlink/></div></section>
 </>;
}
