...
export default function Home(){
 ...
 <div className="container"><div className="home-ad-stack"><ResponsiveLeaderboard slot="home-top"/><AdsterraBanner size="468x60" slot="home-468-top"/><NativeBanner slot="home-native-vertical" variant="vertical"/></div></div>
 <section className="section"><div className="container"><div className="section-header"><div><span className="eyebrow">Explore</span><h2>Three content desks</h2><p className="section-intro">Start with the subject that matches what you are trying to solve.</p></div></div><div className="grid-3">{categories.map(x=><Link className="card category-card" key={x.slug} href={"/category/"+x.slug+"/"}><span className="eyebrow">{x.name}</span><h3>{x.name}</h3><p>{x.short}</p><strong>Explore desk <span aria-hidden="true">→</span></strong></Link>)}</div><div className="home-inline-ad"><AdsterraBanner size="300x250" slot="home-300-middle"/><AdsterraBanner size="468x60" slot="home-468-middle"/></div></div></section>
 <section className="section section-soft" id="latest"><div className="container"><div className="section-header"><div><span className="eyebrow">Latest</span><h2>New guides</h2></div></div><div className="grid-3">{articles.map(x=><article className="card" key={x.slug}><div className="meta">{x.categoryName} · {x.stage}</div><h3>{x.title}</h3><p>{x.description}</p><Link href={"/articles/"+x.slug+"/"}>Read guide <span aria-hidden="true">→</span></Link></article>)}</div><div className="home-bottom-ads"><AdsterraBanner size="160x300" slot="home-160x300"/><AdsterraBanner size="160x600" slot="home-160x600"/></div><Smartlink/></div></section>
 </>;
}
